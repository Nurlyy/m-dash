...
class MainController extends AuthController
{
    ...
    public function actionIndex()
    {
        ...
        $result = json_decode(get_web_page("localhost:8080/backend/main/search?type=index"), true);
    ...
    public function actionCompare()
    {
        ...
        $end_date = isset($_GET['end_date']) ? $_GET['end_date'] : null;
        if (strlen($start_date) < 11 && strlen($end_date) < 11) {
            $result = json_decode(get_web_page("localhost:8080/backend/main/search?type=index&start_date={$start_date}&end_date={$end_date}"), true);
            $this->splitData($result, $start_date);
    ...
    public function actionComparecontent()
    {
        ...
        if (
            ...
            $result = json_decode(get_web_page("localhost:8080/backend/main/search?type=3&start_date={$start_date}&end_date={$end_date}&first={$first}&second={$second}&discussionChart={$discussionChart}&sentimentChart={$sentimentChart}&subsChart={$subsChart}&likesChart={$likesChart}&commentsChart={$commentsChart}&repostsChart={$repostsChart}"), true);
            $this->splitData($result, $start_date);

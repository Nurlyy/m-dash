...
class MainController extends AuthController
{
    ...
    private function splitDayByDay($id, $chartArray, $type)
    {
        if (!empty($chartArray)) {
            $temp = [];
            $donutArray = [];
            foreach ($chartArray[$id] as $key => $value) {
                $prev = 0;
                $counter_for_start_date = 0;
                foreach ($value as $k => $data) {
                    $temp[$id][$key][$k] = (isset($prev) && $prev > 0 && $data > 0) ? (($data - $prev < 0) ? 0 : $data-$prev) : $data;

                    if (!$counter_for_start_date == 0) {
                        if (isset($donutArray[$id][$key])) {
                            $donutArray[$id][$key] += (isset($prev) && $prev > 0 && $data > 0) ? (($data - $prev < 0) ? 0 : $data-$prev) : $data;
                            // $donutArray[$id][$key] += $data-$donutArray[$id][$key];
                        } else {
                            $donutArray[$id][$key] = (isset($prev) && $prev > 0 && $data > 0) ? (($data - $prev < 0) ? 0 : $data-$prev) : $data;
                        }
                    }
                    $counter_for_start_date++;
                    (isset($data) && $data > 0) ? $prev = $data : $prev = $prev;
                }
            }
            if ($type == "chart") {
                return $temp;
            ...
            foreach ($result['city_data'] as $c_data) {
                ...
                if (isset($result['all_data'][$c_data['id']])) {
                    ...
                    foreach ($result['all_data'][$c_data['id']] as $value) {
                        $this->set_data($c_data['id'], $value, explode(" ", $value['date'])[0], $c);
                        $c += 1;
                    }
                    // var_dump($this::$date_posts);exit;
                    // $this::$date_posts = $this->splitDayByDay($c_data['id'], $this::$date_posts, "chart");
                    // $this::$postsSentimentChart = $this->splitDayByDay($c_data['id'], $this::$postsSentimentChart, "chart");
                ...
                if (isset($result['all_data']['subs'])) {
                    ...
                    if(isset($this->splitDayByDay($c_data['id'], $this::$totalSubsChart, "donut")[$c_data['id']])){$this::$totalSubsDonut[$c_data['id']] = $this->splitDayByDay($c_data['id'], $this::$totalSubsChart, "donut")[$c_data['id']];}
                    if(isset($this->splitDayByDay($c_data['id'], $this::$totalSubsChart, "chart")[$c_data['id']])){$this::$totalSubsChart[$c_data['id']] = $this->splitDayByDay($c_data['id'], $this::$totalSubsChart, "chart")[$c_data['id']];}
    ...
    private function set_data($id, $value, $date, $c)
    {
